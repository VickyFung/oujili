          ...
          <div class="config-content">
            ...
            <template v-else-if="group.groupCode === 'security'">
              <n-form :model="configs.security" label-placement="left" label-width="150">
                ...
                <n-divider>后端安全</n-divider>
                <n-form-item label="是否开启拦截">
                  <n-switch v-model:value="configs.security.interceptEnabled" />
                  <span class="form-hint">开启后将对指定接口进行拦截</span>
                </n-form-item>
            ...
            <template v-else-if="group.groupCode === 'tencentyunMp'">
              <n-form :model="configs.tencentyunMp" label-placement="left" label-width="120">
                <n-form-item label="全局人脸启用">
                  <n-switch v-model:value="configs.tencentyunMp.globalFaceEnable" />
                </n-form-item>
                <n-form-item label="规则ID">
                  <n-input v-model:value="configs.tencentyunMp.ruleId" placeholder="请输入腾讯云人脸核身规则ID" />
                </n-form-item>
                <n-form-item label="SecretId">
                  <n-input
                      v-model:value="configs.tencentyunMp.secretId"
                      type="password"
                      show-password-on="click"
                ...
                <n-form-item label="SecretKey">
                  <n-input
                      v-model:value="configs.tencentyunMp.secretKey"
                      type="password"
                      show-password-on="click"
                ...
                <!-- 新增：腾讯云通信配置 -->
                <n-form-item label="IM SDKAppID">
                  <n-input
                      v-model:value="configs.tencentyunMp.sdkAppId"
                      placeholder="请输入腾讯云通信 SDKAppID"
                  />
                </n-form-item>
                <n-form-item label="IM SecretKey">
                  <n-input
                      v-model:value="configs.tencentyunMp.chatSecretKey"
                      type="password"
                      show-password-on="click"
                      placeholder="请输入腾讯云通信密钥"
                  />
                </n-form-item>
                <!-- 新增结束 -->
                <n-form-item label="人脸API地址">
                  <n-input v-model:value="configs.tencentyunMp.faceApi" placeholder="https://face.tencentcloudapi.com" />
                </n-form-item>
                <n-form-item label="重定向URL">
                  <n-input v-model:value="configs.tencentyunMp.redirectUrl" placeholder="核身完成后的跳转地址" />
                </n-form-item>
                <n-divider />
                <n-alert type="info" title="使用说明" :bordered="false">
                  <p>1. 该配置用于腾讯云人脸核身（微信小程序专用）</p>
                  <p>2. 需提前在腾讯云控制台申请规则ID（RuleId）</p>
                  <p>3. SecretId 请使用子账号密钥，遵循最小权限原则</p>
                  <p>4. 人脸API地址默认为 https://face.tencentcloudapi.com，一般无需修改</p>
                  <p>5. 重定向URL用于核身完成后的回调跳转（如需）</p>
                  <p>6. 新增腾讯云通信（IM）配置：SDKAppID 和 IM SecretKey，用于集成聊天功能</p>
                </n-alert>
              </n-form>
            </template>
            <!-- 其他配置 -->
